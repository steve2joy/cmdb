...
## 十一、Phase 5：测试与回归（已完成第一轮，后续运行态缺陷已持续修复）

> 状态：已于 2026-04-01 完成第一轮测试与回归。  
> 当前结论：现有测试集与关键只读/登录链路在 PostgreSQL 下已通过，随后又完成了多轮运行态人工回归与缺陷修复，当前已满足本文定义的 B 级验收；但当前仓库自动化覆盖面有限，尚不能宣称达到 A 级“核心单元测试充分覆盖”。
...
### 11.2.5 2026-04-02 运行态补充验证与缺陷修复

在 Phase 5 第一轮完成后，又通过实际浏览器和接口回归补充发现并修正了以下运行态问题：

- 大量 MySQL 宽松隐式类型转换导致的 PostgreSQL 运行时错误
- 模板导入过程中 `varchar = integer` 类 cache 查询错误
- 拓扑视图中心节点实例搜索中的 `DISTINCT + ORDER BY` PostgreSQL 兼容问题
- CI 创建成功后，实例回读 SQL 中 `text/json` 混合 `UNION` 引发的类型不匹配问题
- CIType 继承查询中模型名字符串与整型 `child_id` 混用问题
- 资源层级中“新增应用无响应”的原版前端 bug
...
- 模板导入/导出链路可用
- 资源数据中创建物理机不再报错
- 拓扑视图中新建拓扑时中心节点实例搜索可用
- 资源层级中新增应用已恢复正常

说明：

- 上述最后三项结果包含浏览器人工验证结论
- 这些补充结果提高了当前 PostgreSQL 分支的可用性，但不改变“自动化覆盖仍偏弱”的整体判断

### 11.2.6 前端构建发布注意事项

后续若手工重构建 `cmdb-ui`，必须注意：

- 仓库中的 `cmdb-ui/.env` 默认包含 `VUE_APP_API_BASE_URL=http://127.0.0.1:5000/api`
- 正式 Docker 构建通过 `docker/Dockerfile-UI` 内的 `sed` 将其改写为相对路径 `/api`
- 若直接执行 `npm run build` / `yarn build` 而未做上述改写，浏览器会绕过 `8000` 的 Nginx 代理直连 `5000`
- 其典型症状是：页面可打开，但登录请求打到 `http://127.0.0.1:5000/api/v1/acl/login` 并返回 `403`

因此，后续本地手工构建 UI 时必须满足以下任一条件：

- 直接走 `docker/Dockerfile-UI`
- 手工覆盖 `VUE_APP_API_BASE_URL=/api`
- 构建后确认最终产物中不再包含 `127.0.0.1:5000`
